...
module.exports = (files, pagesReq) => {
  ...
  const routes = {
    path: prefixLink('/'),
    component: requireComponent(require('content/_template')),
    childRoutes: [],
    indexRoute: {},
    pages,
    templates,
  ...
  staticFileTypes.forEach((type) => {
    try {
      ...
      // console.log('create-routes.js: type:', type, ' not found, e:', e)
  ...
  pages.forEach((p) => {
    ...
    if (staticFileTypes.indexOf(page.file.ext) !== -1) {
      if (_.startsWith(page.path, '/notes/') && _.includes(['org'], page.file.ext)) {
        handler = require('wrappers/note.js')
      } else if (_.startsWith(page.path, '/life/') && _.includes(['org'], page.file.ext)) {
        handler = require('wrappers/life.js')
      } else {
